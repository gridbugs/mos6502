...
pub struct LoadAndStoreAllAddressingModes;
impl Sample for LoadAndStoreAllAddressingModes {
    ...
    fn check_result<M: MemoryReadOnly>(_cpu: &Cpu, m: &M) {
        ...
        for i in 8..12 {
            assert_eq!(m.read_u8_read_only(i), 0xCC);
        }
        for i in 12..16 {
            assert_eq!(m.read_u8_read_only(i), 0xDD);
        }
        for i in 16..20 {
            assert_eq!(m.read_u8_read_only(i), 0xEE);
        }
        for i in 20..24 {
            assert_eq!(m.read_u8_read_only(i), 0xFF);
        }
    }
}
